...
export default function DataPage({ tabela }: { tabela: EntityKey }) {
  ...
  useEffect(() => {
    ...
    Promise.all([api.get(entity.endpoint), api.get(`${entity.endpoint}/campos`)])
            ...
            <table className={styles.table}>
              ...
              <tbody>
                {resultados.map((item, index) => (
                  <tr key={String(item.uuid ?? item.id ?? index)}>
                    {campos.map((campo) => (
